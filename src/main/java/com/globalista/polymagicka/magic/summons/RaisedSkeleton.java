...
public class RaisedSkeleton extends IronGolemEntity implements PolymerEntity {

    private int attackTicksLeft;
    private int lookingAtVillagerTicksLeft;


    public RaisedSkeleton(EntityType<? extends IronGolemEntity> entityType, World world) {
    ...
    public static DefaultAttributeContainer.Builder createAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.MAX_HEALTH, (double)30.0F)
                .add(EntityAttributes.MOVEMENT_SPEED, (double)0.32F)
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        this.playSound(SoundEvents.ENTITY_SKELETON_HURT, 1.0F, 1.0F);
        return bl;
    }
    ...
    public void handleStatus(byte status) {
        if (status == 4) {
            ...
            this.playSound(SoundEvents.ENTITY_SKELETON_AMBIENT, 1.0F, 1.0F);
        } else if (status == 11) {
    ...
    public void playAmbientSound() {
        this.playSound(SoundEvents.ENTITY_SKELETON_AMBIENT);
    }
